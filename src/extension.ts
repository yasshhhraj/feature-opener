...
export function activate(context: vscode.ExtensionContext) {
    ...
    // pin the source file in column 1 first
    await vscode.window.showTextDocument(document, {
        viewColumn: 1,
        preserveFocus: false
    });

    // fetched files start from column 2
	...
	const openImportedFiles = async () => {
		...
		const allImports = await collectImports(document.uri.fsPath, sourceExt, 0, 10, visited);
		collecting.dispose();
